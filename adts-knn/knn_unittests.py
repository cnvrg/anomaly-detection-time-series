import pandas as pd
import unittest
from knn import NoCSVError, ContaminationValueError
from knn import validate_arguments, get_contamination, split_features_labels, map_labels
...
class TestValidateArguments(TestKNN):
...
class TestGetContamination(TestKNN):
    ...
    def test_invalid_contamination(self):
        """Checks for ContaminationValueError when contmination value is out of range"""
        with self.assertRaises(ContaminationValueError):
            get_contamination(self.valid_df, self.positive_label)


class TestSplitFeaturesLabels(TestKNN):
    def test_return_type(self):
